...
The repository ships only source code and the raw Excel file. Everything else —
`outputs/`, `artifacts/`, `retail_cleaned.csv`, `Online Retail Cleaned.xlsx` — is
generated by the scripts and is covered by `.gitignore`, so it stays out of version
control on its own rather than needing to be cleared out by hand.

Re-running the scripts live is the intended way to demonstrate the pipeline.
